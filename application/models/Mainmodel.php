...
class mainmodel extends CI_Model {
    ...
    public function yutubeData() {   //메인 슬라이드
    ...
    public function categoryData1() {   //메인 카테고리
        $sql="SELECT category_idx, category_name, category_image
                FROM callget_category1";
        $list = $this->db->query($sql)->result();
        return array('return'=>true,'list'=>$list);
    }
...
//                limit {$limit} offset {$offset} ";
        ...
        $sql="SELECT product_idx, TO_DAYS(CAST(product_time AS DATE)) -
                TO_DAYS(CAST(now() AS DATE)) AS dates,
                product_get, product_logo, insure_logo,product_thumb,product_img,product_idx2,product_numget,
                product_company, insure_company, insure_name,insure_call, product_name, product_slide, product_thumvideo,product_thumvideo2,
                product_hashtag FROM callget_product
                JOIN callget_insure USING(product_idx)
                WHERE CURDATE() <= product_time ".$where."
                AND product_show = 1
    ...
    public function  callData($idx) {
        $sql="SELECT product_idx, product_slide, product_name, 
                insure_logo, product_logo, insure_company, insure_name,insure_call, insure_video,product_thumvideo,product_thumvideo2,product_gettext,product_hashtag,
